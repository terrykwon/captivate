...
def _get_transform():
    transform_list = []
    transform_list.append(transforms.Resize((input_resolution, input_resolution))) # input_resolution=224
...
class GazeFollower(BasePredictor):
    ...
    def __init__(self, weight_path):
        ...
        if torch.cuda.is_available():
            ...
            pretrained_state_dict = torch.load(weight_path, map_location=torch.device('cpu'))['model']
    ...
    def predict_gaze(self, image, face_bbox):
        ...
        head_channel = imutils.get_head_box_channel(face_bbox[0], face_bbox[1], face_bbox[2], face_bbox[3], width, height,
        ...
        raw_hm, _, inout = self.model(frame, head_channel, head) # 3 inputs to model

        # heatmap modulation
        raw_hm = raw_hm.cpu().detach().numpy() * 255 # detach from computational graph
    ...
    def strongest_pixel(self, width, height, raw_hm):
        ...
        pred_x, pred_y = [pred_x/output_resolution * width, pred_y/output_resolution * height]
