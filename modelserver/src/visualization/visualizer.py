...
class Visualizer:
    ...
    # converts hex to rgb
    COLORS_RGB = {k: tuple(int(v[i:i+2], 16) for i in (1, 3, 5)) for (k,v) in COLORS_HEX.items()}


    def __init__(self, scale=1.0):
        ''' Font sizes and line widths are normalized, i.e. resized in proportion to the image dimensions.
    ...
    def add_captions(self, image, transcript, attended_objects):
        ...
        box = self.write_text(box, transcript, (20, 10), Visualizer.COLORS_RGB['white'])

        objects = ', '.join(attended_objects)
        context_string = 'Context: {}'.format(objects)

        box = self.write_text(box, context_string, (20, 210), Visualizer.COLORS_RGB['pink'])
    ...
    def draw_objects(self, image, bboxes, classes, scores):
        ...
        for i, bbox in enumerate(bboxes):
            classname = classes[i]
            if classname == 'person': # person boxes are annoying, so exclude them for now
                continue
    ...
    def plot_heatmap(self, image, norm_map, raw_hm, face_bbox, inout, plot_image=True):
        ...
        if inout < out_threshold: # needs to be smaller than threshold
            ...
            plt.plot((norm_p[0]*width,(face_bbox[0]+face_bbox[2])/2), 
                    (norm_p[1]*height,(face_bbox[1]+face_bbox[3])/2), '-', color=(0,1,0,1))
